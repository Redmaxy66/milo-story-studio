...
| Animation | Renderer-neutral authority, 24 fps rationale, contiguous in/out frames, 5,760 frames / 240 seconds, one authoritative narration binding, exact non-overlapping shot ranges or no-playback states, no spoken/heard claims in no-playback shots, non-overlapping dialogue windows, SC04 count and SC08 closing-cue isolation, SC05 clap isolation, complete voice dependencies, corrected action order, separated audio/production-note semantics, honest missing-asset states, lip-sync fallback, acceptance/revision/assembly boundaries and no renderer adapter. |
| Prompt bundle | 24 unique prompt IDs, exact source hashes, top-level and prompt-level animation-manifest hash synchronization, future reference-specification IDs without asset claims, approved episode-reference matching without canonisation, unresolved duration mapping, requested/observed separation, null live configuration and no provider authority. |
| Approval package | Six repository-only templates: AP01–AP06 use controlled `APPROVED` decisions with reviewer, actual timestamp and explicit conditions; AP05/AP06 approval does not change their artifact bytes or grant provider, generation, assembly, Phase 4 or A3 authority. |
| Generation gate | Unexecuted five-sheet proposal; one initial and at most one revision operation per sheet; unknown costs and null caps block generation; no image/media generation, provider call, upload, spend or asset approval. |
